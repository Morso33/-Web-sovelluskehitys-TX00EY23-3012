  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      const sorted = [...restaurants].sort((a, b) => {
        ...
        return getDistance(userLat, userLon, latA, lonA) - getDistance(userLat, userLon, latB, lonB);
      });
      ...
      sorted.forEach((r) => {
        table.insertAdjacentHTML(
          ...
          `<tr>
            <td>${r.name}</td>
            <td>${r.address}, ${r.postalCode} ${r.city}</td>
          </tr>`
        );
      });
    },
    () => {
      table.insertAdjacentHTML('beforeend', '<tr><td colspan="2">Sijainnin haku epäonnistui.</td></tr>');
    }
  );
} else {
  table.insertAdjacentHTML('beforeend', '<tr><td colspan="2">Geolocation ei ole tuettu.</td></tr>');
}
